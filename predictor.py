...
CLF_HANDLE=open("Calssifier.pickle","rb")
clf=pickle.load(CLF_HANDLE)
CLF_HANDLE.close()
...
class MyApp(tkinter.Frame):
  ...
  def __init__(self, master):
   ...
   self.grid()
   BGRSS =tkinter.Button(text="USE_BGR", command=screenshotBGR)
   BGRSS.grid(row=0, column=0)
   HSVSS =tkinter.Button(text="USE_HSV", command=screenshotHSV)
   HSVSS.grid(row=0, column=1)
   LABSS =tkinter.Button(text="USE_LAB", command=screenshotLAB)
...
while True:
  ...
  if BGRSS_clicked == 1:
   ...
   prediction=clf.predict(reshape_a)
   printed=""
   if prediction==0:
    printed="Niose"
   elif prediction==5:
   ...
   lmain = tkinter.Label(window,text=printed,font=("Courier", 44))
   lmain.text = printed
   lmain.pack()
   BGRSS_clicked =0
